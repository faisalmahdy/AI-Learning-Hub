...
> **Status: v0.3 — shared understanding reached** (grill-me Rounds 1–2 settled, see [BRAINSTORM.md](BRAINSTORM.md)): self-first · 8–12 h/week · public monorepo of leveled topic modules · GitHub Pages explorer generated from markdown · recall-ledger gate.
...
- A **track** below = a **topic folder** under [`modules/`](modules/) (e.g. `modules/evals-and-statistics/`). There are **9 topics**: the 7 tracks, the cross-cutting ops track, and generative-media (added in Round 2).
- Each module carries a **level tag** — `basic` (understand + reproduce a worked example from the labs) / `intermediate` (build a variation with a measured result) / `advanced` (original work meeting the "defensible claim with uncertainty" standard). Boss-fight recall at every level.
- The Hub **absorbs** teaching material from the labs as de-personalized copies, and curates reliable **external resources** as links + own summaries only (never full copies); every absorbed page carries a source line.
- The reading surface is a **self-contained interactive HTML explorer** generated by `tools/build_site.py` (stdlib-only) and published via **GitHub Pages**; markdown stays the source of truth, views are generated.
...
## Track 8 — Generative media (added in grill Round 2)

**Where you stand.** Real practice, previously homeless in this curriculum: an 11-stage script→export film pipeline that produced an actual short film (Predator Takjil), a manifest-first provenance vault (SHA-256 asset IDs, schema-versioned manifests) from the Genblaze hackathon, and 88K LOC of procedurally generated, CI-verified 3D assets in arena-ai. Gaps: storyboard/prompt quality is never scored (ties to Track 1), in-app generation is manual copy-paste, and the provenance UI still contains a fake `stableHex` placeholder.

| # | Module | Anchor | Build | Definition of done |
|---|--------|--------|-------|-------------------|
| 8.1 | Prompt engines & style anchors | docs/system-prompts.md; director-presets.md (ai-studio) | Extract the Master Visual Anchor pattern into a teachable module | A stranger can produce style-consistent multi-scene prompts |
| 8.2 | Provenance for generated media | worker/genblaze_pipeline.py + storage_b2.py | Replace the fake stableHex with real hashing; document the manifest-first pattern | Provenance chain verifiable end to end |
| 8.3 | Scoring generated media | Arena UI (side-by-side rating) + Track 1 harness | A graded storyboard/prompt eval set with ≥2 raters | First quality numbers for the film pipeline |
| 8.4 | Agent-verified procedural assets | arena-ai rebuild-then-diff CI, headless render checks | Distill the generate-then-prove pattern into a module | Pattern doc + one new asset passing the gates |
